...
- **Avis Google en direct** : le texte des avis est récupéré côté serveur (`lib/google-reviews.ts`, Places API New, cache 6 h) à partir de `GOOGLE_PLACES_API_KEY` et `GOOGLE_PLACE_ID`. Ces deux variables sont à ajouter dans Vercel → Settings → Environment Variables — **sans** préfixe `NEXT_PUBLIC_`, sinon la clé part dans le JavaScript envoyé aux visiteurs. Côté Google Cloud, le projet doit avoir la facturation activée : sans elle l'API renvoie la note et le nombre d'avis mais jamais leur texte, et le site affiche alors les témoignages de secours.
